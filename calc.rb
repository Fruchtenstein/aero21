...
def calcwlog(d)
    ...
    db.query("SELECT runnerid, teamid, 7*goal/365 from runners where teamid>0", :as => :array).each do |r|
        $l.info r
      $l.info ("SELECT COALESCE(SUM(distance),0), COALESCE(SUM(time),0) FROM log, runners WHERE log.runnerid=runners.runnerid AND log.runnerid=#{r[0]} AND date>'#{bow}' AND date<'#{eow}'")
    ...
    $l.info "#{teamdist},#{teamtime},#{teamgoal}"
    teamdist.each do |team, distance|
      if team
        $l.info ("REPLACE INTO teamwlog VALUES (#{team}, #{week_number}, #{distance}, #{teamtime[team]}, #{teamgoal[team]})")
